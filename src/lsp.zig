...
pub const Server = struct {
    ...
    fn getTokenType(kind: evaluator.TokenKind) u32 {
        return switch (kind) {
            ...
            .number => 1, // number
            .string => 2, // string
            .symbol => 2, // string (symbols are now strings)
            .comma, .colon, .colon_colon, .semicolon, .equals, .arrow, .backslash, .dot => 3, // operator
            .plus, .plus_plus, .minus, .star, .slash, .ampersand, .ampersand_ampersand, .pipe_pipe, .bang => 3, // operator
